QoDesk.TiendaWindow = Ext.extend(Ext.app.Module, {
    ...
    createWindow: function () {
        ...
        if (!win) {
            ...
            this.formTiendaDetalle = new Ext.FormPanel({
                id:'formTiendaDetalle',
                ...
                items:[
                    {
                        ...
                        layout:'column', items:this.gridRp
                    },
                    {
                        collapsible:true,
                        collapsedTitle:true,
                        ...
                        layout:'column',
                        tbar:[
                            {text:'Grabar', scope:this, handler:this.grabartienda, iconCls:'save-icon', disabled:true, id:'tb_grabartienda' }
                        ],
                        items:[
                            {
                                frame:true,
                                ...
                                items:[
                                    ...
                                    { xtype:'htmleditor', fieldLabel:'Descripción', name:'descripcion', anchor:'95%', readOnly:false, height:'auto' },
            ...
            win = desktop.createWindow({
                ...
                items: this.formTiendaDetalle
            });
        }
        win.show();

        function cargaDetalle (tienda,forma){
            forma.getForm().load({
                url:'modules/desktop/tienda/server/crudTienda.php?operation=selectForm',
                params:{
                    id:tienda
                }
            });
        ...
        this.gridRp.on('rowclick', function (grid, rowIndex) {
            this.record = this.gridRp.getStore().getAt(rowIndex);
            this.idTiendaRecuperada = this.record.id;

            /*cargar el formulario*/
            cargaDetalle (this.idTiendaRecuperada, this.formTiendaDetalle);
            Ext.getCmp('tb_grabartienda').setDisabled(false);
        }, this);
        ...
        Ext.Msg.show({
            ...
            fn:function (btn) {
                if (btn == 'yes') {
                    ...
                    //window.location.href = 'modules/desktop/tienda/server/crudTienda.php?operation=selectForm?id=' + this.idTiendaRecuperada;

                    var myForm = Ext.getCmp('formTiendaDetalle').getForm();
                    myForm.submit({
                        url : 'modules/desktop/tienda/server/crudTienda.php?operation=updateForm',
                        method : 'POST',
                        fileUpload : true,
